...
아래 표는 현재 `main` 브랜치 기준입니다.

| 항목 | 현재 |
| --- | --- |
| 기술 | Java 17, Spring Boot 4.1.0, Gradle |
| 구현 API | `GET /health` (추가 구현은 Swagger/OpenAPI에서 확인) |
| 계획 API | Wiki API 카탈로그와 관련 Issue에서 설계·추적 |
| 로컬 DB | H2 + Flyway baseline |
| 개발·배포 DB | PostgreSQL + Flyway |
| 보안 | Health·API 문서만 공개, 나머지는 기본 차단(JWT는 #4) |
...
| Security | 세션 없는 REST 방식으로 공개 경로 외 요청을 차단하고, #4에서 JWT와 역할을 연결합니다. | `SecurityConfig` |
