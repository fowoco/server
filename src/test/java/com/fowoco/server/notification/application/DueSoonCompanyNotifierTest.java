...
class DueSoonCompanyNotifierTest {
    ...
    private final DueSoonCompanyNotifier notifier = new DueSoonCompanyNotifier(
            taskRepository, notificationRepository, tenantDatabaseContext, uuidGenerator, clock
    );

    @Test
    void createsNotificationForEachDueSoonTask() {
        Task taskA = dueSoonTask();
        Task taskB = dueSoonTask();
        ...
        notifier.processCompany(COMPANY_A, LocalDate.of(2026, 8, 10), LocalDate.of(2026, 8, 17));

        verify(notificationRepository, times(2)).insert(org.mockito.ArgumentMatchers.any(Notification.class));
    }

    @Test
    void notificationGoesToTaskCreator() {
        Task task = dueSoonTask();
        when(taskRepository.findOpenTasksDueBetween(
                org.mockito.ArgumentMatchers.eq(COMPANY_A),
                org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any()
    ...
    @Test
    void setsTenantContextForTheGivenCompany() {
        when(taskRepository.findOpenTasksDueBetween(
        ...
        verify(tenantDatabaseContext).setCompanyIdForCurrentTransaction(COMPANY_A);
    }

    private Task dueSoonTask() {
        return Task.create(
                UUID.randomUUID(), COMPANY_A, UUID.randomUUID(), UUID.randomUUID(),
